...
- Cointegration 
  - Up to this point I said that we want variables to be I(0) to make regressions (stationary)
  - However, there are cases where we want to regress variables that are I(1) (non-stationary)
  - If they are cointegrated, then we can regress them, because they share a long term relationship.
- Error Correction Models
  - If variables are cointegrated, then we can use ECM to estimate the long term relationship, and the short term dynamics.
  - Useful to see how fast deviations from long term relationship are corrected.
- Forcasting
