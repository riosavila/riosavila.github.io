...
We don't, which is why we can use a normal directly. Instead we use a t-distribution, which uses $se(\beta\hat)$
...
- Something you may or may not have noticed. The significance level $\alpha$ can be choosen by the resercher.
...
- When considering the topic of asymptotic theory, there are few concepts that are important ton consider.
  ...
  2. When samples grow, we are more concern about consistency instead rather than "just" unbiased estimators.
...
- In large samples, this is no longer enough. One requires Consistency!
  ...
  - $p(|\hat \beta - \beta|<\varepsilon) = 1$ or that The variance shrinks to zero. (we can estimate $\beta$ almost surely.
  - This is also known as asymptotic unbiasness.
  
- Interestingly, Consistency can be achieved with a weaker A4': $Cov(e,x)=0$, assuming that we require only linear independence.
...
y=st_data(.,"y")
x=st_data(.,"x1 x2 cns")
xx=cross(x,x);ixx = invsym(xx)
beta = J(5000,3,0)
for(i=1;i<=5000;i++){
	beta[i,]=(ixx*cross(x,y:+runiform(25,1,-sqrt(12)/2,sqrt(12)/2)))'
}
end
getmata beta1*=beta, force


## Is normality assumption important?

- Everything we have seen so far was possible under the normality assumption of the errors.

- if $e$ is normal, then $b$ is normal (even in small samples)

- But what if this assumption fails? would we care?

  - Not really! When samples are large