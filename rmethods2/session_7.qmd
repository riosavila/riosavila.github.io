...
- For the work with LIMEW and LIMPTIP, this has also been used for distributional analysis of employment assigment and services use.
...
- For LIMM, in addition to identifying potential workers, we also need to identify potential job characteristics 
  - Hours of work (full-time, part-time), wage levels, etc
 
- This is done using "Observed" Jobs characteristics of "currently employed" workers.

::: {.callout-note}

This is not the only way to do it. We could make assumptions about wage levels, hours of work, etc. (e.g. minimum wage, full-time, etc.) (Employment guarantee?)
:::

- A probit/logit model would be estimated to predict the likelihood of working, and this will be used to assign jobs.

  - High `p-scores`, first ones to be assigned jobs.
...
- Jobs wages and hours are assigned based on matched data (with donors)
