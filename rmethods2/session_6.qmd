...
mata
   ...
   cn = 0; cnt2=0
   jx = J(h,2,0)
   // go over each observation in "treated"
   for(i=1;i<=h;i++){
       // estimate distance to all observations in "control"
       x1d = x2:-x1[i,]
       // This is an added step for Mahalanobis distance
       dis = rowsum(cross(x1d',v):*x1d)
       // get the index for the closest observation (first)
       ord = order(dis,1)[1]
       // save it
       jx[i,]=ord,dis[ord] 
   }
