...
#| fig-width: 6
#| fig-height: 3
plot(airbnb_holdout$price, airbnb_holdout$price - airbnb_holdout$price_hat, pch = 16, col = "grey50")
```
...
#| fig-height: 3
airbnb_holdout %>%
  mutate(guests_cat = cut(guests, breaks = c(0, 2, 4, 6, 8, 10, 12, 14, 16), labels = 1:8)) %>%
  group_by(guests_cat) %>%
  summarize(price_mean = mean(price), 
            price_hat_mean = mean(price_hat),
            price_hat_lb = quantile(price_hat, 0.1),
            price_hat_ub = quantile(price_hat, 0.9)) %>%
  ggplot(aes(x = guests_cat, y = price_mean)) +
  geom_bar(stat = "identity", fill = "grey80") +
  geom_errorbar(aes(ymin = price_hat_lb, ymax = price_hat_ub), width = 0.2)
```
